...
class testlabRandom(object):
    ...
    # assertion tests
        assert labRandom.random_object()
        assert labRandom.random_number(35)
        assert labRandom.random_bytes(2)
        assert labRandom.random_binary(32)
        assert labRandom.random_fraction()
        assert labRandom.random_integer(-55, 55)
        assert labRandom.random_double(-5.5, 5.5)
        test1 = [1, 2, 3, 4, 5, 6]
        test2 = ['a', 'b', 'c', 'd', 'e', 'f']
        test3 = [{'a'}, {'b'}, {'c'}, {'d'}, {'e'}, {'f'}]
        assert labRandom.random_shuffle(test1)
        assert labRandom.random_shuffle(test2)
        assert labRandom.random_shuffle(test3)
