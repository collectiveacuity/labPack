...
class SQLTable(object):
    ...
    def list(self, filter=None, sort=None, limit=100, cursor=None, ids_only=False):
        ...
        if sort:
            for i in range(len(sort)):
                ...
                for key, value in criterion.items():
                    ...
                    criteria_key = '.%s' % key
                    if criteria_key not in self.model.keyMap.keys():
                        raise ValueError('%s key %s does not exist in record_schema.' % (msg, key))
        else:
            sort = []
        ...
        while True:
            ...
            for record in self.session.execute(select_object).fetchall():
                ...
                # filter results with non-sql supported conditions
                if filter:
                    if self.model.query(filter, record_details):
                        if ids_only:
                            records.append(record_details['id'])
                        else:
    ...
    def export(self, sql_table, merge_rule='skip', coerce=False):
        ...
        if remove_columns or retype_columns or resize_columns:
            if not coerce:
                raise ValueError(
                    "Migration from %s to %s prevented because schemas don't match and data could be lost." % (
